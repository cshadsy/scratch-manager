...
def clone_turbowarp(stdscr):
    install_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "installations", "turbowarp")
    if os.path.exists(install_dir):
        msg = "turbowarp already exists!"
    else:
        stdscr.clear()
        stdscr.addstr(0, 0, "Cloning TurboWarp...\n")
        stdscr.refresh()
        try:
            subprocess.run(["git", "clone", "https://github.com/TurboWarp/scratch-gui.git", install_dir], check=True)
            stdscr.addstr(1, 0, "Running npm install --force...\n")
            stdscr.refresh()
            subprocess.run(["npm", "install", "--force"], cwd=install_dir, check=True)
            msg = "TurboWarp downloaded and dependencies installed!"
        except Exception as e:
            msg = f"Error: {e}"
    stdscr.clear()
    stdscr.addstr(0, 0, msg)
...
def download_menu(stdscr):
    curses.curs_set(0)
    current_row = 0
    header = "==== Scratch Manager: Download Scratch ===="
    while True:
        stdscr.clear()
        h, w = stdscr.getmaxyx()
